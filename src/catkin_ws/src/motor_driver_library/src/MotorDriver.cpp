...
MD::Motor::Motor(int directionPin, int pwmPin, float minSpeed, float maxSpeed) {
...
// --- Wheel direction ---
void MD::Motor::setDirection(float speed) {
    m_direction = speed;
    if (speed >= 0.0) {
...
void MD::Motor::setSpeed(float speed) {
    if (speed < m_minSpeed) {
        speed = m_minSpeed;
...
float MD::Motor::getSpeed() {
    return m_speed;
}

void MD::Motor::setMinSpeed(float minSpeed) {
    m_minSpeed = minSpeed;
}

// int Motor::getMinSpeed() {return 0;}

void MD::Motor::setMaxSpeed(float maxSpeed) {
    m_maxSpeed = maxSpeed;
}
