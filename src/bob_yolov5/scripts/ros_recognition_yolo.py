...
class Camera_subscriber(Node):
    ...
    def __init__(self):
        ...
        if self.classify:
            ...
            self.modelc.load_state_dict(
                torch.load("resnet50.pt", map_location=self.device)["model"]
            ).to(self.device).eval()
    ...
    def camera_callback(self, data):
        ...
        s = np.stack(
            [letterbox(x, self.imgsz, stride=self.stride)[0].shape for x in img], 0
        )  # shapes
        self.rect = (
        ...
        pred = self.model(
            ...
            visualize=(
                increment_path(save_dir / "features", mkdir=True)
                if self.visualize
                else False
            ),
        )[0]
