...
class NMPCController(Node):
    ...
    def __init__(self):
        ...
        self.lidar_angles = np.linspace(-2.5, 2.5, num=self.nscan)
    ...
    def odom_callback(self, msg):
        ...
        if not self.initialized:
            ...
            x_pred, x_pred_a, usol, usol_a, w, s = self.run_open_loop_mpc(
                ...
                self.u_st_0, self.u_st_0_a, self.w_st_0, self.s_st_0, self.pisolver,self.lidar_scans
            )
    ...
    def scan_callback(self, msg):
        ...
        for i in range(len(msg.ranges)):
            if msg.ranges[i] == inf:
                scans.append(np.finfo(np.float32).max)
            else:
    ...
    def run_open_loop_mpc(self, x0, s0, x_st_0, x_st_0_a, u_st_0, u_st_0_a, w_st_0, s_st_0, solver,scan):
        args_p = np.concatenate((x0,np.array(s0),scan),axis=0) # ranges
    ...
    def obstacle(self, x, r, theta): # scan
        ...
        h = fmax((-(x[0]-scanx) ** 2 - (x[1]-scany) ** 2 + 5.0), 0)
        return h

    def obstacle_cost(self, x_ob, x_ob_a,lidar_scans, mu=100):

        V_obs = 0

        for i in range(self.nscan):
            r = lidar_scans[i]
            theta = self.lidar_angles[i]

            h_ob = self.obstacle(x_ob,r,theta)
            h_ob_a = self.obstacle(x_ob_a,r,theta)

            cost = if_else(r<3.0,0.5 * mu * h_ob**2 + 0.5 * mu * h_ob_a**2,0)
    ...
    def objective_cost(self, X, U, W, S_a, X_a, U_a,lidar_scans):
        J = 0.0
        for i in range(self.N):
            ...
            obs_cost = self.obstacle_cost(X[:2, i], X_a[:2, i], lidar_scans)
        ...
        obs_cost = self.obstacle_cost(X[:2, self.N], X_a[:2, self.N], lidar_scans)
    ...
    def Pi_opt_formulation(self):
        ...
        lidar_scans = SX.sym("L", self.nscan)  # Lidar scans

        self.system = Function("sys", [X, U], [self.mobile_robot_ode(X, U)])

        J = self.objective_cost(X, U, W, S_a, X_a, U_a,lidar_scans)
