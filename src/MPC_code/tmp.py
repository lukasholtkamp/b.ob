...
class NMPCController(Node):
    ...
    def __init__(self):
        ...
        self.nscan = 360
        self.lidar_scans = np.zeros(self.nscan)
    ...
    def odom_callback(self, msg):
        ...
        if not self.initialized:
            ...
            x_pred, x_pred_a, usol, usol_a, w, s = self.run_open_loop_mpc(
                ...
                self.u_st_0, self.u_st_0_a, self.w_st_0, self.s_st_0, self.pisolver,np.array([self.min_r]),np.array([self.lidar_angles[self.scan_min_ind]])
            )
    ...
    def scan_callback(self, msg):
        ...
        self.lidar_scans = np.array(scans)
    ...
    def run_open_loop_mpc(self, x0, s0, x_st_0, x_st_0_a, u_st_0, u_st_0_a, w_st_0, s_st_0, solver,scan,theta):
        args_p = np.concatenate((x0,np.array(s0),scan,theta),axis=0) # ranges
    ...
    def obstacle_cost(self, x_ob, x_ob_a,lidar_scan,lidar_theta, mu=10):
        ...
        V_obs = if_else(lidar_scan!=-1,0.5 * mu * h_ob**2 + 0.5 * mu * h_ob_a**2,0)
    ...
    def objective_cost(self, X, U, W, S_a, X_a, U_a,lidar_scan,lidar_theta):
        J = 0.0
        for i in range(self.N):
            ...
            obs_cost = self.obstacle_cost(X[:2, i], X_a[:2, i], lidar_scan,lidar_theta)
        ...
        obs_cost = self.obstacle_cost(X[:2, self.N], X_a[:2, self.N], lidar_scan,lidar_theta)
    ...
    def Pi_opt_formulation(self):
        ...
        lidar_scan = SX.sym("L", 1)  # Lidar scans
        lidar_theta = SX.sym("L_the", 1)  # Lidar scans

        self.system = Function("sys", [X, U], [self.mobile_robot_ode(X, U)])

        J = self.objective_cost(X, U, W, S_a, X_a, U_a,lidar_scan,lidar_theta)
        ...
        vnlp_prob = {
            "f": J,
            "x": Opt_Vars,
            "p": vertcat(P_a,lidar_scan,lidar_theta),
            "g": G_vcsd,
        }
