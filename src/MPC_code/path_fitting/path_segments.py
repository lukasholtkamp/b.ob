...
class PathSegment:
    def __init__(self, start_point=None, end_point=None, segment_type=None):
        self.start_point = np.array(start_point) if start_point is not None else None  # Starting point [x, y]
        self.end_point = np.array(end_point) if end_point is not None else None        # End point [x, y]
    ...
    def straight_line_tf(self,s0,v_max,eps):
        ...
        self.set_transformation_matrix(transformation_matrix(self.rotation,(p1[0]+p2[0])/2,(p1[1]+p2[1])/2))
        

    def parab_tf(self,s0,theta,v_max,A_prime, B_prime, C_prime, D_prime, E_prime, F_prime,eps):
        ...
        s1 = (-((1/np.sin(theta))*np.sqrt(-8*a*h*np.sin(theta) + 8*a*p1[0]*np.sin(theta) + np.cos(2*theta) + 1))/np.sqrt(2) + 2*a*r*s0 + (1/np.tan(theta)))/(2*a)
        s2 = (((1/np.sin(theta))*np.sqrt(-8*a*h*np.sin(theta) + 8*a*p1[0]*np.sin(theta) + np.cos(2*theta) + 1))/np.sqrt(2) + 2*a*r*s0 + (1/np.tan(theta)))/(2*a)
        ...
        check_x = np.abs(ax*self.end_time**2+bx*self.end_time+cx - self.end_point[0])
        check_y = np.abs(ay*self.end_time**2+by*self.end_time+cy - self.end_point[1])
    ...
    def transform_point(self, point, matrix):
        point_homogeneous = ca.vertcat(point[0], point[1], 1)  # Use CasADi vertcat for symbolic points
    ...
    def transform_p(self,point):
        point_homogeneous = np.array([point[0], point[1], 1])  # Use CasADi vertcat for symbolic points
    ...
    def f(self, s):
        ...
        if self.segment_type == 'line':
            x = self.x_coefficients[0]*s**2+self.x_coefficients[1]*s+self.x_coefficients[2]
            y = self.y_coefficients[0]*s**2+self.y_coefficients[1]*s+self.y_coefficients[2]
