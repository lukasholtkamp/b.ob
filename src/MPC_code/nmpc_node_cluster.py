...
class NMPCController(Node):
    ...
    def __init__(self):
        ...
        self.obs_sub = self.create_subscription(Obstacles,'/raw_obstacles',self.obs_callback,10)
    ...
    def odom_callback(self, msg):
        ...
        if not self.initialized:
            ...
            x_pred, x_pred_a, usol, usol_a, w, s = self.run_open_loop_mpc(
                ...
                self.u_st_0, self.u_st_0_a, self.w_st_0, self.s_st_0, self.pisolver,self.obs_list
            )
    ...
    def obstacle_cost(self, x_ob, x_ob_a, obs_list, mu=10):
        ...
        for i in range(self.max_obs):
            ...
            cost = if_else(obs_list[i,2]<=0.0,0.5 * mu * h_ob**2 + 0.5 * mu * h_ob_a**2,0)
