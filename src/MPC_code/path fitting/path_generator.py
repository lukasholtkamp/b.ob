...
# Read the waypoint data from the CSV file
file_path = 'path_data_log.csv'  # Replace with the actual path to your CSV file
n = 20  # Modify as necessary for different step sizes

# Get the selected waypoints and their original indices
original_indices, selected_waypoints, total_waypoints = select_waypoints_with_indices(file_path, n)
...
# Output the indices and processed waypoints
epsilon = 0.15
    ...
    if x_eps_right_prev is None:
        ...
        if processed_indices[i] - processed_indices[i-1] > 20:
            ...
            while processed_indices[i] - processed_indices[i-1] - n*k > 20:
                ...
                next_x = total_waypoints[processed_indices[i-1]+n*k]
...
for i, segment in enumerate(path_segments):
    ...
    if segment.segment_type == 'parabola':
        ...
        if i > 0:  # Ensure we have a previous segment
            ...
            prev_end = prev_segment.end_point
        else:
        ...
        if i < len(path_segments) - 1:  # Ensure we have a next segment
            ...
            next_start = next_segment.start_point
            next_end = next_segment.end_point
        else:
